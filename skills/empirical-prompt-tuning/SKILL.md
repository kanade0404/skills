...
<!-- Source: https://github.com/mizchi/skills/blob/main/empirical-prompt-tuning/SKILL-ja.md -->
...
リポジトリ外の skill を参照しているため、実体は各 plugin / 上流リポジトリ側にある。

- `superpowers:writing-skills`（[obra/superpowers](https://github.com/obra/superpowers) plugin 提供）— skill 作成時の TDD アプローチ。本 skill の「subagent で baseline → 修正 → 再実行」と本質的に同じ
- `retrospective-codify`（[mizchi/skills](https://github.com/mizchi/skills) 上流参考）— タスク後の学び固定化。本 skill はプロンプト開発中、retrospective-codify はタスク終了後、と使い分ける
- `superpowers:dispatching-parallel-agents`（[obra/superpowers](https://github.com/obra/superpowers) plugin 提供）— 複数シナリオを並列で走らせるときの作法
