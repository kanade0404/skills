...
**起票の直後に PR の head を再照合する**。GitHub の PR head は必ず可変な branch なので、「検証した commit を指す」ことを不変 ref で保証することはできない — `gh pr view <URL> --json headRefOid` を読んで検証済み SHA と突き合わせ、動いていれば (取得できなければ) **補償に入る**。順序は **復旧 → 台帳 → close**: `git push --force-with-lease=refs/heads/<branch>:<観測した SHA>` でブランチを検証済み SHA に戻し (lease が弾かれたら remote を 1 度読み直して 1 度だけ再試行) → **戻せたときだけ**その上に台帳の `rejected` を 1 commit 積んで fast-forward で push → `gh pr close` を 3 回まで再試行 → それでも open で head が検証済み SHA でも台帳 commit の先端でもないなら job summary に「手動対応が必要」として出す。**台帳を先に push すると remote 先端が動いて lease が必ず弾かれ、復旧の道が閉じる**ので順序は変えない。この照合は `ledger_id` を持たない reconcile ブランチも通る (台帳の書き込みだけを飛ばす)。ここだけは記録より復旧を優先する — 検証していない commit を指した PR をレビュー面に残す方が高くつくため。検出した件数は run を赤で終わらせる。**窓そのものは残り、PR head に不変 ref を指定できない以上 GitHub 上では閉じ切れない** (`references/scheduling.md`)。
