---
name: pr-monitor
description: "Long-polls a PR you created or shipped until MERGED / CLOSED, handling the CI failures and new review comments that land while it sits unattended. Each poll reads state, checks, and unresolved threads via the bundled `prm status <PR>`; a new failure dispatches `ci-self-heal`, a new thread dispatches `pr-review-respond`, both as Task subagents. Backs off 60s → 1800s, resets on any change, and launches `retro` once the PR settles. Callers dispatch it as a subagent so main stays free. Use right after `shipping` finishes or `gh pr create`, and for 「PR 監視して」「マージされるまで見張って」「マージ/クローズしたら振り返りまで回して」「CI とコメントも見張って対応まで回して」. Not for: fixing or waiting out a red build now (`ci-self-heal`), answering comments now (`pr-review-respond`), shipping end to end (`shipping`), merging the PR, or production / service monitoring. It only waits for the outcome."
claudecode:
  allowed-tools:
    - Read
    - Write
