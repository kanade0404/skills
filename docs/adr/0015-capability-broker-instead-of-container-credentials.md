...
### 試行の記帳は write-ahead で、多く数える側に倒す

受理試行の回数を上限にするなら、**その回数が正しく数えられていなければ上限は意味を持たない**。ところが
「push を受けたこと」と「判定を権威面に記録したこと」は別の書き込みなので原子的ではなく、間で落ちると
試行が記録から消える。そこで **記帳を副作用の前に置く** — 既存の write-ahead 規範
([ADR 0012](0012-write-authority-by-lease-and-sha-cas.md)) と同じ流儀である。

- **検査を始める前に** `kind=policy_decision` を「試行を開始した」状態で CAS 追記し、**検査の後に、
  結果 (allow / deny・落ちた検査) を同じ試行 id を持つ別レコードとして追記する**。
- **確定はレコードの上書きではなく追記である。** 既存レコードを書き換えると、**派生表示 (issue の
  コメント等) が開始のまま取り残される** — 派生の再同期は event id で冪等化して既存なら skip する契約
  なので ([ADR 0011](0011-authority-state-in-dedicated-state-repo.md))、id が変わらない更新は派生側に
  届かない。追記なら終結イベントが新しい id を持つので、派生同期の契約を変えずに表示が追いつく。
  これは [ADR 0012](0012-write-authority-by-lease-and-sha-cas.md) が release を DELETE ではなく
  `status=released` の追記にしたのと同じ判断であり、**events を append-only に保つ既存規範とも揃う**。
  なお**権威面は上書きでも正しかった** — これは表示が実態から遅れる問題であって、判定が誤る問題では
  ない。それでも直すのは、可監査性が 1 位だからである ([ADR 0009](0009-ility-priority-order.md))。
- **落ちたら多く数える**。開始は記録されたが終結レコードが無い試行は、**1 回の試行として数える**。上限の
  目的は有界性なので、**undercount (試行が消えて上限をすり抜ける) より overcount (実際より多く数えて
  早めに止まる) の方が安全側**である。どちらに倒すかを決めずに実装すると、クラッシュのたびに上限が
  緩む方向にずれる。試行数は**開始レコードの数**で数えるので、追記に変えても数え方は変わらない。
- **冪等性は試行 id で担保する** — 受信した push の SHA と試行の連番から作る id を開始レコードと終結
  レコードの両方に刻む。同じ id の開始レコードが既にあれば追記しない (リトライで二重計上しない)。同じ
  id の終結レコードが既にあれば追記しない (二重の終結を作らない)。

### 超過の正当性は、落ちた検査の種別から機械的に分類する

上限超過を再考のトリガに使う以上 (下記)、「正当な作業が上限に当たった」のか「上限が意図どおり働いた」
のかを**後から機械的に区別できる**必要がある。`policy_decision` は既に「どの検査で落ちたか」を持って
いるので、新しいスキーマを足さずに次の 2 分類を導く。

- **敵対寄り (再考の理由にしない)**: secret 検出・保護パス・`codex/**` 外・TOCTOU。**これらでの停止は
  防壁が働いた記録**であって、値が小さすぎる証拠ではない。
- **中立 (人間の判断が要る)**: サイズ・受理試行回数・ディスクサイズの各上限。**量的な上限は、正当な
  大きな作業でも暴走でも同じ形で当たる**ため、記録だけからは決まらない。
- 中立に分類された超過は `needs-human` に倒れるので、**その場の人間の判断 (正当な作業だったか) を
  needs-human の決着記録に 1 フィールドとして残す**。トリガ判定はこのフィールドだけを見る。これ以上の
  分類語彙は作らない — 必要なのはトリガを引くか引かないかの二値である。
...
**②のディスク上限が指す「量」を 1 つに固定する** — 上限で止めるものと、p95 で測るものが違う量だと、
閾値監視が上限の妥当性を何も語らなくなる。**測る量は lane の bare repo ディレクトリ全体の実使用量**で、
**objects・refs に加えて quarantine と受信中の一時ファイルを含む**。ディスクを実際に食うのはこの合計
だからである。上限強制も p95 集計も、この同じ量を見る。
...
「敵対的入力からの防壁」ではない**という書き分けをする。**両者は同じ量 (上記のディレクトリ全体) を、
違う時点で見ている** — 1 GiB の上限は定常状態の総量に対する閾値で、超過は `needs-human`。fs / cgroup の
quota は受信の瞬間に効く即時の遮断で、pre-receive の判定を待たない。役割が違うだけで、測る対象は 1 つ
である。拒否時に quarantine が破棄される挙動は git の
version に依存するため、**対象 version での実挙動を PoC ② で確認する**。

**②の 2 つの数値は暫定値である** — ただし「根拠のない仮置き」ではなく、**運用データで再導出することを
前提にした初期値**であり、見直しの条件をここで定義する。他の運用パラメータ (tick・TTL・thread 時間・
同時 thread 数など) は Charter (契約配布物) の rules で宣言される既定値群が値を持っているが、**受理試行
回数と bare repo のディスクサイズはその既定値群に未収録**なので、本 ADR が初期値を置いた。Phase 1 の実測
(通常の lane が何回の push で決着するか、成果物の実サイズ) で確定し、**確定時は同じ既定値群 (rules) に
収録して ADR 側から数値を落とす**。

**再考のトリガを時点ではなく状態で定義する。**「Phase 1 で確定」だけでは、Phase 1 を過ぎた後に値が現実と
ずれても誰も気づかない。以下のいずれかが観測されたら値を再導出する。

- **分布が閾値に迫ったとき** — 定常運転における受理試行数と bare repo サイズの **p95 が上限の 50% を
  超えたら**、上限が実際の作業を切りかけているとみなして再導出する。50% を切っている限り、上限は
  「正常系には当たらない防壁」として機能している。**集計の契約を決めておく** — 対象は**直近 30 日に
  決着した lane**、**最小標本数 20 lane**、それ未満なら**判定を保留してトリガを引かない** (標本が
  少ないうちは p95 が跳ねるため)。`needs-human` で止まった lane も母集団に含める — 除くと上限に当たった
  ケースだけが消えて分布が実際より小さく見える。**30 日 / 20 lane も暫定値**で、上の 2 値と同じ再考
  トリガの対象とする。
- **上限超過による `needs-human` が正当な作業で起きたとき** — 即座に再考する。**上限が正当な開発ループを
  誤って止めたことの直接の証拠**だからである。**敵対的入力や暴走による超過は再考の理由にしない** — それは
  上限が意図どおり働いた記録であって、値が小さすぎる証拠ではない。この 2 つを取り違えると、攻撃を受ける
  たびに防壁を緩めることになる。両者は超過を起こした lane の判定記録から区別する。
- **確定して rules に収録した後も、同じトリガを適用しつづける。** 収録は「値が正しくなった」ことを意味
  せず、値の所有者が ADR から rules に移ったことを意味するだけである。

**値そのものより「上限があり、超過が fail closed になり、値が現実とずれたら気づける」ことが Driver の
要求**であり、初期値のまま実装してよい。
...
- broker の判定は、**受理試行 1 回ごとに `kind=policy_decision` として lane の `state.json` に CAS 追記
  する** — allow / deny の別・適用したポリシー版・対象 SHA・どの検査で落ちたかを含め、**secret の値と
  payload 本文は含めない**。受理試行の回数と成否がそのまま権威面で数えられるので、上限の運用状況を
  追加のインフラなしで観測できる ([ADR 0016](0016-quantum-scoped-fitness-functions.md) の測定源)。
  「例外領域ゼロ」
