...
- **検査を始める前に** `kind=policy_decision` を「試行を開始した」状態で CAS 追記し、**その後で検査し、
  結果 (allow / deny・落ちた検査) を同じレコードに確定として書く**。
- **落ちたら多く数える**。開始だけ記録されて結果が無いレコードは、**1 回の試行として数える**。上限の
  目的は有界性なので、**undercount (試行が消えて上限をすり抜ける) より overcount (実際より多く数えて
  早めに止まる) の方が安全側**である。どちらに倒すかを決めずに実装すると、クラッシュのたびに上限が
  緩む方向にずれる。
- **冪等性は試行 id で担保する** — 受信した push の SHA と試行の連番から作る id をレコードに刻み、同じ
  id の再記帳は追記せず既存レコードを更新する。リトライで二重計上しない。
