...
**②の 2 つの数値は暫定値である** — 設計資料は「上限を置くこと」を決めているが値を確定していない。
Phase 1 の実測 (通常の lane が何回の push で決着するか、成果物の実サイズ) で確定する。**値そのものより、
「上限があり超過が fail closed になる」ことが Driver の要求**であり、暫定値のまま実装してよい。
