...
- **scan の実装が 1 つの関数に集まる**。worker の代行経路 (書き込み前の予防) と Fable の起票経路
  (取り込み時の検出) は**当てる時点が違うだけで、検査ロジックは同一のものを共有する**ので、ルールの
  二重管理が生じない。**ただし集まるのは実装であって強制の強度ではない** — 経路ごとに予防か検出かが
  異なることは Decision に書いたとおりで、「検査が 1 つだから守りも 1 つ」とは読まないこと。
