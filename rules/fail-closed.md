...
root: false
targets: ["*"]
description: "矛盾するフラグ・検証不能な入力・失敗しうる外部操作を黙って通す/捨てる/続行する fail-open を禁止し、明示 exit・escalate・エラー行 emit の fail-closed を既定とする規律。スクリプト・skill・ワークフローの設計と実装すべてで常時適用する。"
globs: ["**/*"]
---
