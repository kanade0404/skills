...
- **必ず Task ツールで subagent を dispatch** し、白紙でレビュー資料を読ませる。
...
- **設計の "提案" は出さない。** 提案は `software-design` の担当。本スキルは "読んで指摘" のみ。
- **コードを書き換えない。** Fix は提案テキスト。実装は別ステップ / 別スキル。
- **書き手（同セッションの主エージェント）に評価させない。** 必ず subagent dispatch。
