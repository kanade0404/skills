...
//
// NOT generated by this script (hand-maintained, committed, and deliberately left
// alone by both write and --check mode): `.claude/agents/` (repo-local agent
// overrides) and `.gemini/settings.json`. The latter points Gemini CLI's
// `context.fileName` at the generated root `AGENTS.md` — rulesync has no
// `gemini` target, so that wiring cannot be generated and must be written by
// hand (see skills/rulesync-sync/SKILL.md). Neither path is staged into
// `.rulesync/` or produced into the scratch output root; see `MIRRORED_DIRS`
// below for how the stale sweep stays clear of them too.
//
...
// itself, the hand-maintained `.claude/agents/` overrides, or the gitignored
// runtime state under `.claude/.pr-monitor/`. Hand-maintained config outside
// these roots entirely (`.gemini/settings.json`) is likewise never walked.
...
// `commands/` and `hooks/` are placeholder-only (a README without frontmatter) and
// would fail rulesync parsing, so they are not staged. `subagents/` is a different
// case: it does hold real canonical definitions (one frontmatter-bearing file per
// agent), and it is left unstaged *deliberately* — staging it (together with adding
// `subagents` to `--features` below) would make `generate` write `.claude/agents/`,
// overwriting the hand-maintained repo-local agent overrides kept there (header
// comment above).
