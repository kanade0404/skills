...
description: >-
  Lens set for design judgments and artifacts — domain model, module boundaries,
  error strategy, architecture decisions, security. 13 lenses: philosophy of
  software design, immutable data model, TM法, functional programming, DDD, TDD,
  Railway Oriented Programming, architecture characteristics, xUnit Test
  Patterns, CQRS, Event Sourcing, ADR, Secure by Design. Use for modeling before
  coding, review of an existing design, transaction / side-effect / consistency
  boundaries, tech-debt priority, security review, and for
  「設計どうする」「ドメインモデル作って」「集約をどこで切る」「Result 型に直したい」「例外やめて Railway で」「CQRS
  入れる?」「Event Sourcing 採用する?」「ADR 書く」「アーキテクチャ決定を残す」「immutable
  データモデルに直して」「TM法でモデル化して」「TDD で進めたい」「Deep Module になってる?」「サブドメイン分けたい」「Secure by
  Design 観点で見て」. Not for: test code review (`test-review`), research
  (`research-practices`), skill authoring (`skill-builder`), blind review of a
  finished design (`design-review`), or implementation chores (typo, lint,
  simple bug fix, perf tuning).
allowed-tools:
  - Read
  - Write
  - Edit
