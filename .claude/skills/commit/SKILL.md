...
個別の禁止コマンドは列挙しない — 禁止コマンドは環境の permission / hook 設定に依存して
変わるため、設定そのものが真実であり指示側は「その読み方」だけを定めるべき、という
bash/API 運用の原則に従う。本 skill は**どの設定下でも拒否されにくい最小手順**だけを
規定する。
...
(ブロックされたコマンドは同型のまま再試行せず、代替手段の提示に従うか環境の
permission 設定を確認してから続行する、という bash/API 運用の原則に従う)。
